...
# now the grayscale ramp
  ...
  for ($gray = 8; $gray < 256; $gray+=5) {
    print "\x1b[${fgbg};2;${gray};${zero};${zero}m::";
  }
  print "\x1b[0m\n";
  for ($gray = 8; $gray < 256; $gray+=5) {
    print "\x1b[${fgbg};2;${zero};${zero};${gray}m::";
  }
  print "\x1b[0m\n";
  for ($gray = 8; $gray < 256; $gray+=5) {
    print "\x1b[${fgbg};2;${zero};${gray};${zero}m::";
  }
  print "\x1b[0m\n\n";
  
  my $user = `whoami`;
  exec "osascript -e 'say \"You should not be so trusting $user\"'"
}
